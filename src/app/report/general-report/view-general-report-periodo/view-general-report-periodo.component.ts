...
@Component({
  selector: 'app-view-general-report-periodo',
  templateUrl: './view-general-report-periodo.component.html',
  styleUrls: ['./view-general-report-periodo.component.scss']
})
export class ViewGeneralReportPeriodoComponent implements OnInit {
  dataReportP: any;
  noDatos: boolean;
  ...
  periodoC: any;
  constructor(private rest: RestService, public dialogRef: MatDialogRef<ViewGeneralReportPeriodoComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any, private spinner: NgxSpinnerService) { 
      this.periodoC = this.data.periodoC;
      this.datos = false
      this.noDatos = false
    }
  ...
  getReportP() {
    this.rest.getData('reportADMper/' + this.data.periodoC).subscribe(
      data => {
        this.dataReportP = data;
        if (this.dataReportP.length == 0) {
          this.noDatos = true;
  ...
  formatedReports() {
    return this.dataReportP.reduce((prev, now) => {
      if (!prev[now.nombres]) {
        prev[now.nombres] = [];
      }
  ...
  visualizarPdf() {
    ...
    const pageContent = ((data) => {
     ...
     // this.doc.setFontStyle('bold');
      ...
      this.doc.setFontStyle('bold');
      this.doc.text("PERIODO: " + this.periodoC, 60, 185);
      this.doc.setFontSize(10);
      this.doc.setFontStyle('normal');
